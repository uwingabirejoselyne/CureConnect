    ...
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400'>
      ...
      <ul className='hidden md:flex items-start gap-5 font-medium'>
        <NavLink to='/'>
            ...
            <hr className='border-none outline-none h-0.5 bg-[var(--bg-primary)] w-3/5 m-auto hidden'/>
        </NavLink>
        <NavLink to='/doctors'>
      ...
      <div className='flex items-center gap-4'>
        {
          ...
          <button onClick={()=>navigate('/login')} className='bg-[var(--bg-primary)] text-white px-8 py-3 rounded-full font-light md:block' >Create account</button>
        }
      </div>
    </div>
  )
